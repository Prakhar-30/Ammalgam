...
- Verify health factor is below threshold: `getUserProtection()`
- Check token approvals are sufficient
- Ensure pair is monitored: `isPairMonitored()`
- Confirm subscription: `isUserSubscribedToPair()`

**Insufficient Funds:**
- Increase token balance for protection asset
- Increase approval amount for callback contract
- Check `maxProtectionAmount` is reasonable

**Position Not Found:**
- Verify you have an active borrowing position in the pair
- Ensure correct pair address in subscription
- Check position exists: call pair's `getInputParams()`

## Production Deployment

**Before Going Live:**
1. Deploy contracts with correct constructor parameters
2. Add all relevant Ammalgam pairs to monitoring
3. Calculate correct event topic hashes for your network
4. Test with small positions and amounts first
5. Verify gas limits are sufficient
6. Monitor system events and success rates

**System Requirements:**
- Users need sufficient balance of protection assets
- Token approvals must be granted to callback contract
- Borrowing position must exist in monitored Ammalgam pair
- Reactive Network (Kopli) connectivity required

The system operates **fully automatically** once configured, providing comprehensive protection against all Ammalgam liquidation types with intelligent subscription management and real-time event response.
